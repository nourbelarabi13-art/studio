...
export default function ReadingView({ id }: { id: string }) {
  ...
  const progressRef = useMemoFirebase(() => {
    if (!db || !user || !id || id === 'manifest') return null;
    return doc(db, "users", user.uid, "progress", id);
  }, [db, user, id]);
  ...
  useEffect(() => {
    if (db && id && id !== 'manifest') {
      incrementNovelView(db, id);
    }
  }, [db, id]);
  ...
  const handleLike = () => {
    ...
    setIsLiking(true);
    toggleLikeNovel(db, id, user.uid, currentUserProfile.username)
